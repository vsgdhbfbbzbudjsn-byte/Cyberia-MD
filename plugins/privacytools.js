...
📌 *Note*: Some commands are owner-only`;
        ...
        await conn.sendMessage(
            from,
            {
                image: { url: `https://files.catbox.moe/ygfz2e.jpg` }, // Replace with privacy-themed image if available
                caption: privacyMenu,
                contextInfo: {
...
async (conn, mek, m, { from, isOwner, reply }) => {
    ...
    try {
        ...
        reply(`📋 \`Drakonis-MD BLOCKED USERS (${count})\`:\n\n${list}`);
    } catch (err) {
        console.error(err);
